  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-[#1E1E2E]">Reported Problems & Tickets</h1>
        <p className="text-xs text-slate-500 font-medium">Review and resolve reported student issues.</p>
      </div>
            ...
            <div key={report._id} className="card-visual p-6 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <div className="text-xs font-extrabold text-[#1E1E2E]">{report.student?.name || 'Student'}</div>
                    <div className="text-[11px] font-medium text-slate-500">{report.student?.email}</div>
                  </div>
                </div>
              ...
              <p className="text-xs font-medium text-slate-800 leading-relaxed">{report.description}</p>
                ...
                <div className="flex items-center gap-3 pt-2">
                  {report.images.map((img, idx) => (
                    <a key={idx} href={img} target="_blank" rel="noreferrer" className="relative group">
                      <img src={img} alt="Evidence" className="w-16 h-16 rounded-2xl object-cover border border-slate-200" />
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2 text-xs">
                ...
                {report.status !== 'resolved' && (
                  <button
                    ...
                    className="btn-visual bg-[#1FAE64] text-white hover:bg-[#1FAE64]/90 text-xs px-4 py-1.5"
                  >
